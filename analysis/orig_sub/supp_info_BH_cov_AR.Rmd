...
par_jags <- c("alpha","ln_BH_a","E_BH_a","beta","gamma","phi",
              "Sp","tot_ln_Rec","lp_age","lp_esc","res_ln_Rec")
...
init_vals <- function() {
	list(alpha=2, gamma=0.1, phi=0.7,
...
for(i in 1:n_cov) {
  ...
  bh_fits[[i]] <- do.call(jags.parallel, mod_jags)
}
```
...
for(i in 1:n_mods) {
  ldens <- cbind(bh_fits[[i]]$BUGSoutput$sims.list$lp_age,
                 bh_fits[[i]]$BUGSoutput$sims.list$lp_esc)
  WAIC[i] <- waic(ldens)$waic
  LOO[i] <- -2*waic(ldens)$elpd_loo
  cc_smry_1[i,] <- bh_fits[[i]]$BUGSoutput$summary["gamma", c("2.5%","50%","97.5%")]
}
...
## B-H alpha
phi_est <- mod_fit$BUGSoutput$sims.list$phi
phiCI <- quantile(phi_est,c(0.025,0.5,0.975))
hist(phi_est,freq=FALSE,xlab="",main="",breaks=seq(-50,50)/50,
     col=clr, border="blue3", ylab="", cex.lab=1.2, yaxt="n")
aHt <- (par()$usr[4]-par()$usr[3])/12
arrows(phiCI,par()$usr[3],phiCI,par()$usr[3]-aHt,
       code=1,length=0.05,xpd=NA,col="blue3",lwd=1.5)
mtext(expression(AR(1)~coefficient~(phi)), 1, line=3, cex=1)
```
...
```{r plot_run_size, fig.width=4, fig.height=3, fig.pos="placeHere"}

...
init_vals_AR <- function() {
	list(alpha = 10,
	     ...
	     pi_tau = 1,
	     pi_eta = rep(1,A),
	     pi_vec = matrix(c(0.005,0.3,0.485,0.15,0.04,0.02), n_yrs-age_min+n_fore, A,
	                     byrow = TRUE),
	     Rec_mu = log(1000),
	     Rec_sig = 0.1,
...
mod_fits[[1]] <- try(do.call(jags.parallel, mod_jags))
## B-H
mod_jags$inits <- init_vals_AR
mod_jags$model.file <- "IPM_BH_AR.txt"
mod_fits[[2]] <- try(do.call(jags.parallel, mod_jags))

## ----JAGS_IO_2, message = FALSE, warning = FALSE, cache = TRUE-----------
## 1. data to pass to JAGS
...
for(i in 1:n_cov) {
  ...
  mod_fits[[cnt]] <- try(do.call(jags.parallel, mod_jags))
  cnt <- cnt + 1
  ## B-H with cov
...
## ----get_LOO_via_loo, warnings = FALSE, messages = FALSE-----------------
n_mods <- length(mod_fits)
LOO <- vector("numeric",n_mods)
cc_smry_1 <- matrix(NA,n_mods,3)
colnames(cc_smry_1) <- c("lo","med","up")
...
## ----plot_S_R, fig.width = 8, fig.height = 5, fig.pos = "placeHere"------
...
## ----plot_cov_effects, fig.width = 8, fig.height = 4, fig.pos = "placeHere", warnings = FALSE, messages = FALSE----
clr <- rgb(0, 0, 255, alpha = 50, maxColorValue = 255)
offSet <- 0.07
...
## ----plot_run_size, fig.width = 6, fig.height = 4.5, fig.pos = "placeHere"----
...
## ----plot_innovations, fig.width = 6, fig.height = 4, fig.pos = "placeHere"----

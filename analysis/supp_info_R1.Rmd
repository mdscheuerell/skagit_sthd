...
if(n_fore > 0) {
  dat_age <- rbind(dat_age,
                   matrix(0,n_fore,A,dimnames=list(n_yrs+seq(n_fore),colnames(dat_age))))
}
...
init_vals_AR <- function() {
	list(alpha=3,
	     ...
	     pi_tau=1,
	     pi_eta=rep(1,A),
	     pi_vec=matrix(c(0.02,0.3,0.47,0.15,0.04,0.02),n_yrs-age_min+n_fore,A,byrow=TRUE),
	     Rec_mu=log(1000),
	     Rec_sig=0.1,
...
for(i in 1:n_cov) {
  ...
  mod_cvrs <- scl_cvrs[, i, drop = FALSE]
  ## cnt & time stamp
...
```{r get_WAIC_via_loo, warnings=FALSE, messages=FALSE}
n_mods <- length(mod_fits)
WAIC <- vector("numeric",n_mods)
LOO <- vector("numeric",n_mods)
cc_smry_1 <- matrix(NA,n_mods,3)
...
d_LOO <- round(LOO-min(LOO),1)
wt_LOO <- exp(-0.5*LOO)/sum(exp(-0.5*LOO))
tbl_WAIC_2 <- data.frame(errors=c(2,2,rep(c(4,4),n_cov)),
                         model=rep(c("Ricker","B-H"),n_mods/2),
                         covar=c(c(NA,NA),rep(colnames(scl_cvrs),ea=2)),
                         WAIC=round(WAIC,1),
                         d_WAIC=d_WAIC,
...
# write.csv(tbl_WAIC,  row.names = FALSE,
#           file = "Willamette_Chin_SR_flow_models_mainstem_model_selection_all.csv")
# best_idx <- which(tbl_WAIC_2$d_WAIC==0)
# tbl_WAIC[,!(names(tbl_WAIC) %in% c("lag","WAIC"))]
tbl_WAIC_2[order(tbl_WAIC_2$WAIC, tbl_WAIC_2$LOO),]
```
...
mod_cvrs <- scl_cvrs[,tbl_WAIC[best_idx,"covar"]]
...
## plot covar ts
plot(t_idx, dat_cvrs[seq(length(t_idx)),"Hrel"], xlab="Brood year", ylab="",

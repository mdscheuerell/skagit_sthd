...
Here are schematics of the deterministic forms of process models used in the analyses.

```{r model_forms, fig.height=6.5, fig.width=3.25, fig.pos="placeHere", fig.align="center"}
## params
## Ricker
ra <- 3
rb <- 1.2e-4
...
`r fig_cap(cnt, caption = "Time series of the estimated total population size (catch plus the adults that escaped to spawn). The observed data are the points; the solid line is the median estimate and the shaded region represents the 95% credible interval.", display = "full")`
...
```{r plot_cov_effects, fig.width=8, fig.height=10, fig.pos="placeHere", warnings=FALSE, messages=FALSE, fig.align="center"}
clr <- rgb(0, 0, 255, alpha = 50, maxColorValue = 255)
xoffSet <- 0.04
yoffSet <- 0.03
...
cnt <- cnt + 1
```
`r fig_cap(cnt, caption = "Time series of the environmental covariates used in the model (a-d), and their estimated effects on population productivity (e-g). Small arrows under histograms denote 2.5th percentile, 50th percentile, and 97.5th percentile of the posterior distribution.", display = "full")`
...
```{r plot_ref_pts, fig.width=8, fig.height=4, warning=FALSE, fig.pos = "placeHere", fig.align="center"}
...
points(x = x_lp, y = yld_prop-0.3,
       pch=21, cex=3.5, col="white", bg="white")

...
## escapement
dat_esc <- read_csv(paste0(ex_url,fn_esc)) dat_hrel <- read_csv(file.path(datadir, "/skagit_sthd_hrel.csv"))
...
We will standardize the covariates to have zero-mean and unit-variance.
...
#      Rec[t,a] <- max(1,tot_Rec[t] * pi_vec[t,a]);
  ...
  for(i in 1:(age_max-age_min-age_skip)) {
    ...
    lp_age_l[i] <- logdensity.multi(dat_age[i,1:A],age_v[i,1:A],dat_age[i,A+1]);
    lp_age[i] <- max(1, lp_age_l[i]);
  }
...
#      Rec[t,a] <- max(1,tot_Rec[t] * pi_vec[t,a]);
  ...
  for(i in 1:(age_max-age_min-age_skip)) {
    ...
    ## multinomial for age comp
    dat_age[i,1:A] ~ dmulti(age_v[i,1:A],dat_age[i,A+1]);
    lp_age[i] <- max(-20, logdensity.multi(dat_age[i,1:A],age_v[i,1:A],dat_age[i,A+1]));
  }
...
```{r JAGS_IO_1, message = FALSE, warning = FALSE, cache = FALSE}
## empty list for fits

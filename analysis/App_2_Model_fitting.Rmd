...
```{r diag_Ricker}
par_conv <- c("alpha","beta","sigma_r","sigma_s","pi_tau",paste0("pi_eta[",seq(A),"]"))
...
```{r jags_IO_covars, message = FALSE, warning = FALSE, cache = FALSE}
## 1. Data to pass to JAGS
dat_jags <- list(dat_age = dat_age,
                 ln_dat_esc = ln_dat_esc,
                 dat_harv = dat_harv,
...
LOO <- data.frame(both=rep(NA, n_mods), age=rep(NA, n_mods), esc=rep(NA, n_mods))
...
for(i in 1:n_mods) {
  ...
  ## calculate LOOIC
  LOO[i,"both"] <- -2*waic(tmp_lp)$elpd_loo
  LOO[i,"age"] <- -2*waic(tmp_lp[,grep("age",colnames(tmp_lp))])$elpd_loo
  LOO[i,"esc"] <- -2*waic(tmp_lp[,grep("esc",colnames(tmp_lp))])$elpd_loo
  if(i < 3) {

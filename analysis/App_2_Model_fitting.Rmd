...
", file=file.path(jagsdir, "IPM_RK_cov2_AR.txt"))
```
...
## Beverton-Holt with 1 covar & AR(1)
...
## Beverton-Holt with 2+ covars & AR(1)
...
", file=file.path(jagsdir, "IPM_BH_cov2_AR.txt"))
```
...
```{r jags_IO_base}
## empty list for fits
mod_fits <- vector("list", 2*(1+n_cov+3))
...
```{r jags_IO_covars, message = FALSE, warning = FALSE, cache = TRUE, eval=TRUE}
## function for inits
...
## Only monitor pointwise likelihoods for now
...
for(i in 1:n_cov) {
  ...
  ## cnt & time stamp
  cat("Count = ", i, "; Time = ", round(((proc.time()-timer_start)/60)["elapsed"], 1), "\n",
      file = "cnt_time.txt", append = TRUE, sep = "")
  ## Ricker with cov
  mod_fits[[i*2+1]] <- fit_jags("IPM_RK_cov_AR.txt", dat_jags, par_jags,
                              init_vals_cov, mcmc_ctrl)
  ## B-H with cov
  mod_fits[[i*2+2]] <- fit_jags("IPM_BH_cov_AR.txt", dat_jags, par_jags,
                              init_vals_cov, mcmc_ctrl)
}
```
...
```{r jags_IO_covars_2, message = FALSE, warning = FALSE, cache = TRUE, eval=TRUE}
## function for inits
init_vals_cov2 <- function() {
	list(alpha = 5,
	     beta_inv = exp(mean(ln_dat_esc, na.rm = TRUE)),
	     gamma = rep(0,3),
	     pi_tau = 10,
...
cset2 <- c("NPGO","STI","Hrel")
dat_jags$n_cov <- length(cset2)

## counter for model indexing
cnt <- 13

## fit models
for(i in 1:3) {
  ## get covar of interest
  dat_jags$mod_cvrs <- scl_cvrs[, c("flow_wtr","flow_sum",cset2[i])]
  ## cnt & time stamp
  cat("Count = ", cnt, "; Time = ", round(((proc.time()-timer_start)/60)["elapsed"], 1), "\n",
      file = "cnt_time.txt", append = TRUE, sep = "")
  ## Ricker with cov
  mod_fits[[cnt]] <- fit_jags("IPM_RK_cov2_AR.txt", dat_jags, par_jags,
                              init_vals_cov2, mcmc_ctrl)
  ## B-H with cov
  mod_fits[[cnt+1]] <- fit_jags("IPM_BH_cov2_AR.txt", dat_jags, par_jags,
                              init_vals_cov2, mcmc_ctrl)
  cnt <- cnt + 2
}
```
...
Via `waic()` with delta values and covariate effects.

```{r get_WAIC, warning=FALSE, cache=TRUE}
n_mods <- length(mod_fits)
WAIC <- data.frame(WAIC_both=rep(NA, n_mods),
                   WAIC_age=rep(NA, n_mods),
                   WAIC_esc=rep(NA, n_mods))
...
for(i in 1:n_mods) {
  ...
  ## calculate WAIC
  WAIC[i,"WAIC_both"] <- waic(tmp_lp)$waic
  WAIC[i,"WAIC_age"] <- waic(tmp_lp[,grep("age",colnames(tmp_lp))])$waic
  WAIC[i,"WAIC_esc"] <- waic(tmp_lp[,grep("esc",colnames(tmp_lp))])$waic
  if(i < 3) {
    cc_smry_1[i,] <- rep(NA,3)
  }
  if(i > 3 & i <= 12) {
      cc_smry_1[i,] <- summary(mod_fits[[i]])$quantiles["gamma", c("2.5%","50%","97.5%")]
      # cc_smry_1[i,] <- mod_fits[[i]]$BUGSoutput$summary["gamma", c("2.5%","50%","97.5%")]
  }
  if(i > 12) {
      cc_smry_1[i,] <- summary(mod_fits[[i]])$quantiles["gamma[3]", c("2.5%","50%","97.5%")]
  }
}

## delta-WAIC
dWAIC <- round(t(t(WAIC) - apply(t(WAIC), 1, min)), 1)
colnames(dWAIC) <- c("dWAIC_both","dWAIC_age","dWAIC_esc")

## wt_LOO <- exp(-0.5*LOO)/sum(exp(-0.5*LOO))
## Table of model selection results
tbl_WAIC <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                  covar = c(c(NA,NA), rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
                  round(WAIC,1),
                  dWAIC,
                  round(cc_smry_1, 2))
tbl_WAIC
...
# best_idx <- which(tbl_LOO$dLOO==0)
# tbl_LOO[,!(names(tbl_LOO) %in% c("lag","WAIC"))]
# tbl_LOO[order(tbl_LOO$LOO),]
```

Via `waic()` and `compare()` with full table of results.

```{r get_WAIC_c, warning=FALSE, cache=TRUE}
WAIC_both <- WAIC_age <- WAIC_esc <- vector("list", n_mods)
## extract log densities from JAGS objects
for(i in 1:n_mods) {
  ## convert mcmc.list to matrix
  tmp_lp <- as.matrix(mod_fits[[i]])
  ...
  ## calculate WAIC
  WAIC_both[[i]] <- waic(tmp_lp)
  WAIC_age[[i]] <- waic(tmp_lp[,grep("age",colnames(tmp_lp))])
  WAIC_esc[[i]] <- waic(tmp_lp[,grep("esc",colnames(tmp_lp))])
}

## WAIC for all data
tbl_WAIC_both <- round(compare(x = WAIC_both), 2)
rownames(tbl_WAIC_both) <- sub("model", "", rownames(tbl_WAIC_both))
tbl_WAIC_both <- tbl_WAIC_both[order(as.numeric(rownames(tbl_WAIC_both))), ]
tbl_WAIC_both <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                       covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
                       as.data.frame(tbl_WAIC_both))
tbl_WAIC_both[order(tbl_WAIC_both[,"waic"]), ]

## WAIC for age data only
tbl_WAIC_age <- round(compare(x = WAIC_age), 2)
rownames(tbl_WAIC_age) <- sub("model", "", rownames(tbl_WAIC_age))
tbl_WAIC_age <- tbl_WAIC_age[order(as.numeric(rownames(tbl_WAIC_age))), ]
tbl_WAIC_age <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                      covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
                      as.data.frame(tbl_WAIC_age))
tbl_WAIC_age[order(tbl_WAIC_age[,"waic"]), ]

## WAIC for escapement data only
tbl_WAIC_esc <- round(compare(x = WAIC_esc), 2)
rownames(tbl_WAIC_esc) <- sub("model", "", rownames(tbl_WAIC_esc))
tbl_WAIC_esc <- tbl_WAIC_esc[order(as.numeric(rownames(tbl_WAIC_esc))), ]
tbl_WAIC_esc <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                      covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
                      as.data.frame(tbl_WAIC_esc))
tbl_WAIC_esc[order(tbl_WAIC_esc[,"waic"]), ]
```

## Using LOOIC

Via `loo()` with delta values and covariate effects.

```{r get_LOOIC, fig.height=4, fig.width=7, warning=FALSE, cache=TRUE}
## empty df for LOOIC 
...
par(mfrow=c(1,2), mai=c(1,1,0.1,0.1), omi=rep(0,4))
...
## delta-LOOIC
dLOO <- round(t(t(LOO) - apply(t(LOO), 1, min)), 1)
colnames(dLOO) <- c("dLOO_both","dLOO_age","dLOO_esc")

## wt_LOO <- exp(-0.5*LOO)/sum(exp(-0.5*LOO))
## Table of model selection results
tbl_LOO <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                 covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
                 round(LOO,1),
                 dLOO,
                 round(cc_smry_1, 2))
...
```{r get_LOOIC_c, warning=FALSE, cache=TRUE}
...
tbl_LOO_both <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                      covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
...
tbl_LOO_age <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                     covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
...
tbl_LOO_esc <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                     covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2), rep(cset2,ea = 2)),
...
At the moment this is for the last model only (i.e., B-H with hatchery releases as a covariate).
...
Now we'll refit the best model and save all of the parameters/states of interest. In this case, we'll select the model with a Ricker process model and the following 3 covariates: summer flow, winter flow, and NPGO.

```{r refit_best_mod, cache = TRUE}
best_idx <- 13
dat_jags$mod_cvrs <- scl_cvrs[,c("flow_wtr","flow_sum","NPGO")]
## params/states to return
par_jags <- c("alpha","E_Rkr_a","mu_Rkr_a",
              "beta",
              "gamma",
              ...
              "pi_eta","pi_tau",
              "sigma_r","sigma_s","res_ln_Rec")
best_fit <- fit_jags("IPM_RK_cov2_AR.txt", dat_jags, par_jags, init_vals_cov2, mcmc_ctrl)
```
...
```{r tbl_summary_stats}
tbl_smry <- summary(best_fit)$quantiles[par_conv,
                                        c("2.5%","50%","97.5%")]
round(tbl_smry, 2)
...
```{r plot_S_R, fig.width = 8, fig.height = 5, fig.pos = "placeHere", eval=FALSE}

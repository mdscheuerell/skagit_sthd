...
cset <- c("flow_wtr", "flow_sum", "Hrel", "NPGO")
...
tbl_WAIC <- cbind(model = rep(c("Ricker","B-H"),n_mods/2),
                  covar = c(c(NA,NA), rep(colnames(scl_cvrs),ea = 2), rep(cset,ea = 2)),
                  round(WAIC,1),
                  dWAIC,
                  round(cc_smry_1, 2))
tbl_WAIC
...
par_jags <- c("alpha","E_BH_a","ln_BH_a",
              ...
              "pi_eta","pi_tau",
              "sigma_r","sigma_s","res_ln_Rec")
best_fit <- fit_jags("IPM_BH_cov_AR.txt", dat_jags, par_jags, init_vals_cov, mcmc_ctrl)
```

...
# return(do.call(rbind.data.frame, cs))

m1 <- as.matrix(mod_fits[[1]])
head(sort(m1[,"lp_age[1]"]), 20)
head(sort(m1[,"lp_age[2]"]), 20)

lp1 <- m1[,grepl("lp_", colnames(m1))]
# apply(is.infinite(lp1), 2, sum)

rm1 <- t(apply(m1, 2, quantile))
rm1[grepl("lp_", rownames(rm1)),]


# cn <- colnames(mod_fits[[1]])
...
#   df[i, -1] <- apply(sapply(mod_fits[[1]][,i], quantile), 1, mean)
# }
# df
```
...
cbind(model = rep(c("Ricker","B-H"),n_mods/2),
      covar = c(c(NA,NA),rep(colnames(scl_cvrs),ea = 2)),
      as.data.frame(tbl_WAIC_both))
      
## WAIC for age data only
tbl_WAIC_age <- round(compare(x = WAIC_both), 2)
...
tbl_WAIC_esc <- round(compare(x = WAIC_both), 2)
...
## WAIC for age data only
tbl_LOO_age <- round(compare(x = LOO_both), 2)
...
## WAIC for escapement data only
tbl_LOO_esc <- round(compare(x = LOO_both), 2)
...
```{r lp_violins, fig.height=4, fig.width=7, eval=FALSE}
## base plot

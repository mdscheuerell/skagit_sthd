...
cc_smry <- data.frame(covariate = cset,
                      ...
                      BHmd = rep(NA,4),
                      BH = rep(NA,4))
## grab posterior summaries
for(i in 1:4) {
  cc_smry[i,2:4] <- summary(mod_fits[[3]])$quantiles[paste0("gamma[",i,"]"),
                                                     c("2.5%","50%","97.5%")]
  cc_smry[i,5:7] <- summary(mod_fits[[4]])$quantiles[paste0("gamma[",i,"]"),
                                                     c("2.5%","50%","97.5%")]
}
cc_smry
```
...
```{r refit_best_mod, cache = TRUE, eval=FALSE}
## params/states to return

...
model {
	...
	E_ln_Rec[1] <- ln_Sp[1] + ln_Rkr_a[1] - beta*Sp[1] + phi*innov_1;
	...
	for(t in 2:(n_yrs-age_min+n_fore)) {
		...
		E_ln_Rec[t] <- ln_Sp[t] + ln_Rkr_a[t] - beta*Sp[t] + phi*res_ln_Rec[t-1];
...
init_vals <- function() {
	list(mu_Rkr_a=1, gamma=rnorm(n_cov,0,0.1),
...
pDat <- apply(mod_fit$BUGSoutput$sims.list$ln_s,2,sort)
pDat <- apply(pDat,2,function(x) { x[mcmc_samp*CI_vec] })
pDat[2,] <- apply(mod_fit$BUGSoutput$sims.list$ln_s,2,median)
ypMin <- min(pDat)
ypMax <- max(pDat)
par(mai=c(0.8,0.8,0.1,0.1), omi=c(0,0.2,0.1,0.2))

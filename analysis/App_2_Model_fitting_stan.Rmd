...
hist_treedepth <- function(fit, plot.hist=TRUE) { 
  sampler_params <- get_sampler_params(fit, inc_warmup=FALSE) 
  hist(sapply(sampler_params, function(x) c(x[,'treedepth__']))[,1],
       breaks=0:20, main="", xlab="Treedepth", plot=plot.hist) 
...
```{r pair_plots, fig.width=7, fig.height=7}
pairs(mod_fits[[1]], pars = c("log_a", "b", "phi", "sigma_proc", "sigma_obs"))
```

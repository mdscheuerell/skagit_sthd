...
idx_NA_yrs <- which(dat_age$sum<AA, TRUE)
...
if(AA!=dim(dat_age)[2]) {
	stop("data file must have AA=(age_max-age_min+1) age categories")
}
...
# harvest
dat_harv <- read.csv(file_harv)

# drop year col & first age_max rows
dat_harv <- c(dat_harv[,-1],rep(0,n_fore))
#dat_harv <- rep(0,n_yrs+n_fore)

# covariate(s)
dat_cov <- read.csv(file_cov)

# drop year col & first age_max rows
dat_cov <- dat_cov[,cov_in]

# z-score covariates
dat_cov <- scale(dat_cov)
```
...
init_Stan <- function() 
{
  ...
  RR <- matrix(NA, n_yrs-age_min+n_fore, AA)
  age_mean <- apply(dat_age,2,sum)/sum(apply(dat_age,2,sum))
  for(tt in 1:(n_yrs-age_min+n_fore)) {
    RR[tt,] <- (dat_esc[tt] + dat_harv[tt])*age_mean
    }
  R_tot <- rpois(n_yrs-age_min+n_fore,1.5e4)
  mu_p <- colSums(dat_age)/sum(dat_age)
  return(list(log_a = runif(1,1,3),
...
par_Stan <- c("log_a","b","K","pp","sigma_proc","sigma_obs","R_tot","Rec","spawners","mu_p","prec_p")

# fit the model in Stan & store results
stan.sksh <- stan(file = file.path(standir, "IPM_RK_AR.stan"),
                  data = dat_Stan, 
                  init = init_Stan, 
                  pars = par_Stan,
                  control=list(adapt_delta=0.95, max_treedepth=11),
                  chains = 4, iter = 5000, warmup = 1000, thin = 10, cores = 4)
```
...
summary(do.call(rbind, args = get_sampler_params(stan.sksh, inc_warmup = FALSE)), digits = 2)

# HMC sampler stats: each chain separately
lapply(get_sampler_params(stan.sksh, inc_warmup = FALSE), summary, digits = 2)

# number of post-sampler divergent transitions
count_divergences(stan.sksh)

#hist_treedepth(stan.sksh)


mod.list <- As.mcmc.list(stan.sksh)





Rec <- extract(stan.sksh, pars = "Rec", permuted = TRUE, inc_warmup = TRUE)$Rec
R_tot <- unlist2(extract(stan.sksh, pars = "R_tot", permuted = TRUE, inc_warmup = TRUE))
pp <- extract(stan.sksh, pars = "pp", permuted = TRUE, inc_warmup = TRUE)$Rec




m1 <- as.array(stan.sksh)
#dev.new()
par(mfrow=c(4,1), mai=c(0.6,0.6,0.1,0.1))
apply(m1[,,"log_a"],2,acf)


m2 <- as.matrix(stan.sksh)
#dev.new()
pairs(stan.sksh, pars=c("b","log_a","K"))

pairs(stan.sksh, pars=c("mu_p","prec_p"))

pairs(stan.sksh, pars=c("sigma_proc","sigma_obs"))

traceplot(stan.sksh, pars=c("sigma_proc","sigma_obs"))

#traceplot(stan.sksh, pars=colnames(m2)[grep("Rec",colnames(m2))])

get_posterior_mean(stan.sksh, par=c("K","log_a","sigma_proc","sigma_obs"))

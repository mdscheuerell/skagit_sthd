...
init_Stan <- function() 
{
  ...
  RR <- matrix(NA, TT-age_min+n_fore, AA)
  age_mean <- apply(dat_age,2,sum)/sum(apply(dat_age,2,sum))
  for(tt in 1:(TT-age_min+n_fore)) {
    RR[tt,] <- (dat_esc[tt] + dat_harv[tt])*age_mean
    }
  R_tot <- rpois(TT-age_min+n_fore,1.5e4)
  ...
  return(list(log_a = runif(1,1,3),
              ...
              mu_p = mu_p,
              prec_p = runif(1,1,10),
              pp = matrix(mu_p,TT-age_min+n_fore,AA,byrow=TRUE),
...
stan.sksh <- stan(file = "Skagit_steelhead_SSM.stan",
                  ...
                  chains = 4, iter = 11000, warmup = 1000, thin = 40, cores = 4)
```

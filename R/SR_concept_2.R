...
# par(mai=c(0.5,0.5,0.5,0.1), omi=c(0.1,0.1,0,0.1+0.5))

## Ricker
plot(ss, rr, type="n", xlim=range(ss), ylim=range(ss), xaxs="i", yaxs="i",
     xlab="", ylab="", xaxt="n", yaxt="n", bty="L")
mtext(expression(italic(S[t])), 1, line=1, cex=1.1, at=12000)
mtext(expression(italic(R[t])), 2, line=0.5, cex=1.1, at=12000, las=1)
# rttl <- expression((a)~Ricker:~italic(R[t])==frac(italic(alpha * S[t]),italic(e^{beta * S[t]})))
rttl <- "(a) Ricker"
text(400, 12000, rttl, cex=1.1, adj=c(0,1), xpd=NA)
## 1:1
...
text(12300, ra*12000/exp(rb*12000), rmod, adj=c(0,0.5), xpd=NA)
## alpha
...
mtext(expression(italic(S[t])), 1, line=1, cex=1.1, at=12000)
mtext(expression(italic(R[t])), 2, line=0.5, cex=1.1, at=12000, las=1)
# bttl <- expression((b)~Beverton-Holt:~italic(R[t])==frac(italic(alpha * S[t]),1+italic(beta * S[t])))
bttl <- "(b) Beverton-Holt"
text(400, 12000, bttl, cex=1.1, adj=c(0,1), xpd=NA)
## 1:1
abline(a=0, b=1, col="gray")
#text(1.2e4, 1.2e4, "1:1", adj=c(1,0))
## R-S
...
# text(12000, ba*12000/(1 + bb*12000), bmod, pos=4, xpd=NA)
text(12300, ba*12000/(1 + bb*12000), bmod, adj=c(0,0.5), xpd=NA)
## alpha

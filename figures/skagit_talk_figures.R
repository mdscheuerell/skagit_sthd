...
## ----model_forms, fig.height=6.5, fig.width=3.25, fig.pos="placeHere", fig.align="center"----
## params
## Ricker
ra <- 3
rb <- 1.2e-4
...
png(file.path(figsdir, "Fig_1a_Ricker_form.png"),
    height = 6, width = 6, units = "in", res = 500)

par(mai=c(0.6,0.6,0.2,0.2), omi=c(0,0,0,0.25))
...
png(file.path(figsdir, "Fig_1b_B-H_form.png"),
    height = 6, width = 6, units = "in", res = 500)

par(mai=c(0.6,0.6,0.2,0.3), omi=c(0,0,0,0.4))
...
## ----plot_run_size, fig.width = 6, fig.height = 4.5, fig.pos = "placeHere", fig.align="center"----
png(file.path(figsdir, "Fig_2_total_run_size.png"),
    height = 6, width = 10, units = "in", res = 500)

par(mai = c(0.9,0.9,0.1,0.1), omi = c(0.1,0.1,0.1,0.1))
...
plot(t_idx_f, p_dat[3,], ylim = c(yp_min,yp_max), type = "n",
     ...
     xlab = "Year", ylab = "Catch + escapement", main = "", cex.lab = 1.5)
polygon(c(t_idx_f, rev(t_idx_f)), c(p_dat[3,], rev(p_dat[1,])),
        col = clr, border = NA)
lines(t_idx_f, p_dat[2,], col = "blue3", lwd = 2)
points(t_idx_f, exp(ln_dat_esc) + dat_harv, pch = 16, cex = 1.2)
axis(1, at = seq(1980, 2015, 5),  cex.axis = 1.2)
axis(2, at = c(4000, 8000, 16000),  cex.axis = 1.2)

dev.off()


## ----plot_S_R, fig.width = 8, fig.height = 5, fig.pos = "placeHere", fig.align="center"----
png(file.path(figsdir, "Fig_3_SR_relationship.png"),
    height = 6, width = 10, units = "in", res = 500)
...
plot(s_dat[2,], r_dat[2,], xlim = c(0,xM), ylim = c(0,yM), type = "n",
     xaxs = "i", yaxs = "i", cex.lab = 1.5, cex.axis = 1.2,
     xlab = expression(Spawners~(10^3)),
     ylab = expression(Recruits~(10^3)),
     xaxt = "n", yaxt = "n", bty="L")
axis(1, at = seq(0,xM,dd*2), labels = seq(0,xM,dd*2)/1000, cex.axis = 1.2)
axis(2, at = seq(0,yM,dd*2), labels = seq(0,yM,dd*2)/1000, cex.axis = 1.2, las=1)
...
hist(R_b_est, freq = FALSE, breaks = brks, col = clr, border = "blue3",
     xlab = "", xaxt = "n", yaxt = "n",
     main = "", ylab = "", cex.lab = 1.5, cex.axis = 1.2)
...
mtext(expression(paste("Carrying capacity (",italic(K),", ",10^3,")")),
      side = 1, line = 3, cex = 1.3)

dev.off()


## ----plot_cov_effects, fig.width=8, fig.height=10, fig.pos="placeHere", warnings=FALSE, messages=FALSE, fig.align="center"----
png(file.path(figsdir, "Fig_4_covar_effects.png"),
...
for(i in 1:n_cov) {
  ...
  if(i == n_cov) { mtext(side = 1,"Effect size", line = 3, cex = 1.2) }
}
...
## ----plot_ref_pts, fig.width=8, fig.height=4, warning=FALSE, fig.pos = "placeHere", fig.align="center"----
png(file.path(figsdir, "Fig_6_ref_points.png"),
    height = 6, width = 10, units = "in", res = 500)
...
matplot(SS, OYP, type="l", lty="solid", las=1,
        col=c("slateblue","blue","darkblue"), lwd=2,
        xlab="Spawners", ylab="Probability of X% of MSY",
        cex.lab=1.5, cex.axis = 1.2,
        main="", ylim=c(0,1))
points(x = x_lp, y = yld_prop-0.32,
       pch=21, cex=5.5, col="white", bg="white")
text(x = x_lp, y = yld_prop-0.32, paste0(yld_prop*100,"%"),
     col=c("slateblue","blue","darkblue"), cex=1)
...
matplot(Pr_over, type="l", las=1, lwd=2, lty="solid", col=c("slateblue","blue","darkblue"),
        ylab="Probability of overfishing", cex.lab = 1.5, cex.axis = 1.2,
...
points(x=x_lp, y=rep(0.5,3), pch=21, cex=5, col="white", bg="white")
